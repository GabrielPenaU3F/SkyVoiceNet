...
class ContourExtractor:
    ...
    def extract_contour(self, audio, sr=16000, num_bins=512, mode='spec', trim=True):
        ...
        # Remove trailing silences (optional)
        if trim is True:
            audio, _ = librosa.effects.trim(audio, top_db=silence_threshold, hop_length=hop_length)

        # Normalize to [-1, 1]
        audio = audio / np.max(np.abs(audio))

        # Convert to torch tensor with the necessary dimensions
        audio_torch = torch.from_numpy(audio)[None, :].to(device='cuda')
        ...
        contour = torchcrepe.filter.median(contour, win_length=filter_win_frames)
        if mode == 'spec':
            contour = self.build_contour_spectrogram(contour, num_bins=num_bins)
