...
f0rmses_5 = df5["F0 RMSE (Hz)"].to_numpy()

# Índices
idx_min = np.argmin(f0rmses_5)
idx_max = np.argmax(f0rmses_5)
idx_med = np.argmin(np.abs(f0rmses_5 - f0rmses_5.mean()))
...
wavfile.write(os.path.join(path, 'outputs', 'statistics', 'min_f0rmses_orig.wav'), 16000, audio_min_orig)
wavfile.write(os.path.join(path, 'outputs', 'statistics', 'min_f0rmses_pred5.wav'), 16000, audio_min_pred5)

wavfile.write(os.path.join(path, 'outputs', 'statistics', 'max_f0rmses_orig.wav'), 16000, audio_max_orig)
wavfile.write(os.path.join(path, 'outputs', 'statistics', 'max_f0rmses_pred5.wav'), 16000, audio_max_pred5)

wavfile.write(os.path.join(path, 'outputs', 'statistics', 'med_f0rmses_orig.wav'), 16000, audio_med_orig)
wavfile.write(os.path.join(path, 'outputs', 'statistics', 'med_f0rmses_pred5.wav'), 16000, audio_med_pred5)
